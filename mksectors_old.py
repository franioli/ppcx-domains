...
def vectorize_clusters(
    ...
    *,
    ...
    elif method == "smoothify":
        return _vectorize_smoothify(
            cluster_grid,
            X,
            Y,
            buffer_distance=buffer_distance,
...
def _vectorize_smoothify(
    ...
    """Backend: Smoothify method (dual-grid smoothing)."""
    ...
    try:
        ...
        raise ImportError(
            "smoothify, affine, rasterio, and geopandas libraries required. "
            "Install with: pip install smoothify affine rasterio geopandas"
        ) from exc

    # 1. Setup Transform and Mask from Meshgrids
    # We calculate pixel size (dx, dy) directly to preserve direction (sign)
    # X and Y are expected to be 2D meshgrids
    if X.shape[1] > 1:
    ...
    # 2. Vectorize Grid (Raster -> Vectors)
    # Filter out NaNs and ensure we are working with IDs
    mask = ~np.isnan(cluster_grid) & (cluster_grid >= 0)

    # rasterio.features.shapes returns a generator of (geojson, value)
    shapes_gen = rasterio.features.shapes(cluster_grid, mask=mask, transform=transform)

    # Create GeoDataFrame
    try:
        polygons_list = [
            {"geometry": shapely_shape(geom), "cluster_id": int(val)}
            for geom, val in shapes_gen
        ]
    except Exception as e:
        logger.error(f"Error during rasterio vectorization: {e}")
        return SectorPolygons()

    if not polygons_list:
        logger.warning("No valid clusters found to vectorize.")
        return SectorPolygons()

    gdf = gpd.GeoDataFrame(polygons_list)

    # Dissolve by cluster_id to merge fragmented parts of the same cluster into MultiPolygons
    gdf_initial = gdf.dissolve(by="cluster_id", as_index=False)

    logger.debug(f"Vectorized {len(gdf_initial)} unique clusters (pre-smoothing).")

    # 3. Apply Smoothify
    # segment_length matches raster resolution for faithful smoothing
    try:
        smoothed_gdf = smoothify(
            gdf_initial,
            segment_length=abs(dx),
            smooth_iterations=3,
            merge_collection=True,  # Ensures topology is shared between adjacent clusters
            merge_field="cluster_id",
        )
    except Exception as e:
        logger.error(f"Smoothify failed: {e}. Falling back to unsmoothed shapes.")
        smoothed_gdf = gdf_initial

    # 4. Build SectorPolygons output
    polygons = SectorPolygons()
    geometries: dict[str, Polygon] = {}
    for _, row in smoothed_gdf.iterrows():
        cluster_id = row["cluster_id"]
        # Skip background/errors if any (though mask handled this largely)
        if cluster_id < 0:
            continue
        ...
        # Optional: Apply additional user-requested buffer/simplify (e.g. shrinking)
        # Note: smoothify already simplifies, but buffer_distance might be used for margins
        poly = _apply_smoothing(
            geom, buffer_distance=buffer_distance, simplify_tolerance=simplify_tolerance
        )

        # _apply_smoothing helper also handles MultiPolygon -> largest Polygon conversion
        # checking return validity
        if poly is None or poly.area <= 0:
            continue

        # Store in SectorPolygons container
        geometries[label_str] = poly
        # Extract exterior coordinates for plotting/export
        polygons[label_str] = np.asarray(poly.exterior.coords)[:-1]

    polygons.geometries = geometries
    logger.info(f"Created {len(polygons)} polygons using smoothify")
    return polygons
