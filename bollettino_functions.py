...
def resolve_input_dir(date: str | datetime, dir_pattern: str) -> Path:
...
def find_geojson_dates(
    geojson_dir: Path,
    pattern: str = r"^(\d{4}-\d{2}-\d{2})_sectors_polygon\.geojson$",
) -> list[str]:
    """
    Scan a kinematic_sectors_geojson directory and return sorted date strings
    for which a complete polygon+points pair exists.

    Parameters
    ----------
    geojson_dir : Path
        Path to the directory containing the geojson files
        (e.g. output/2016_PPCX_Tele/kinematic_sectors_geojson).
    pattern : str
        Regex to extract the date from the polygon filename.

    Returns
    -------
    list[str]
        Sorted list of date strings ("YYYY-MM-DD") that have both files.
    """
    rx = re.compile(pattern)
    dates = []
    for f in sorted(geojson_dir.iterdir()):
        m = rx.match(f.name)
        if m:
            date_str = m.group(1)
            # reuse shared helper instead of repeating .exists() checks
            if _has_complete_sector_pair(geojson_dir, date_str):
                dates.append(date_str)
            else:
                logger.warning(
                    f"Polygon found for {date_str} but points file is missing — skipping."
                )
    return sorted(dates)
...
def find_closest_date_with_sectors(
    ...
    Returns
    -------
    (found_date, search_dir) : (tuple[datetime | None, Path | None])
    ...
    def _has_sectors(candidate: datetime) -> Path | None:
        ...
        return (
            search_dir if _has_complete_sector_pair(search_dir, candidate_str) else None
        )

    def _search(start: datetime, step: int) -> tuple[datetime | None, Path | None]:
...
def load_sectors_results_from_geojson(
    input_dir: str | Path,
    min_date: str | datetime | None = None,
    max_date: str | datetime | None = None,
    set_crs: str | None = None,
    to_crs: str | None = None,
    force_crs_to_none: bool = False,
) -> list[dict[str, Any]]:
    """Discover and load per-date GeoJSON results, optionally filtering by date range.

    This scans  for matching per-date GeoJSON pairs
    (polygon + points), loads each pair via `load_sectors_by_date` and returns a
    list of dicts with keys: ``date``, ``sectors``, ``pts_by_sector``.

    Args:
        input_dir: Top-level run directory (Path or string).
        min_date: Optional minimum date (inclusive) to keep; accept a ``YYYY-MM-DD`` string or ``datetime``.
        max_date: Optional maximum date (inclusive) to keep; accept a ``YYYY-MM-DD`` string or ``datetime``.
        geojson_subdir: Subdirectory name that holds per-date GeoJSON files.
        set_crs: If provided, set this CRS on loaded GeoDataFrames (overrides existing).
        to_crs: If provided, reproject loaded GeoDataFrames to this CRS. Requires input CRS present.
        force_crs_to_none: If True, forcibly remove any CRS from loaded GeoDataFrames.

    Returns:
        A list of dictionaries (one per loaded date) with keys:
        - ``date``: date string "YYYY-MM-DD"
        - ``sectors``: GeoDataFrame with sector polygons and stats
        - ``pts_by_sector``: GeoDataFrame with point-level observations

    Raises:
        FileNotFoundError: If the geojson directory does not exist.
        ValueError: If ``min_date`` is after ``max_date``.
    """
    geojson_dir = Path(input_dir)
    if not geojson_dir.exists():
        raise FileNotFoundError(f"GeoJSON directory not found: {geojson_dir}")

    dates = find_geojson_dates(geojson_dir)
    logger.info(
        f"Found {len(dates)} date(s) with complete GeoJSON pairs in {geojson_dir}"
    )

    # Parse and validate date range arguments
    min_dt: datetime | None = _parse_date(min_date) if min_date is not None else None
    max_dt: datetime | None = _parse_date(max_date) if max_date is not None else None
    ...
    if min_dt is not None or max_dt is not None:
        ...
        for d in dates:
            d_dt = _parse_date(d)
            if min_dt is not None and d_dt < min_dt:
                continue
            if max_dt is not None and d_dt > max_dt:
                continue
            filtered.append(d)
        logger.info(
            f"Filtered dates by range: {min_dt.date() if min_dt else '-inf'} -> "
            f"{max_dt.date() if max_dt else '+inf'}; {len(filtered)}/{len(dates)} kept"
        )
        dates = filtered

    results_list: list[dict[str, Any]] = []
    for date_str in dates:
        sectors_gdf, points_gdf = load_sectors_by_date(
            date_str,
            geojson_dir,
            set_crs=set_crs,
            to_crs=to_crs,
            force_crs_to_none=force_crs_to_none,
        )
        if sectors_gdf is None or points_gdf is None:
            logger.warning(f"Skipping {date_str} due to missing files.")
            continue
        ...
        results_list.append(
            {
                "date": date_str,
                "sectors": sectors_gdf,
                "pts_by_sector": points_gdf,
            }
        )

    logger.info(f"Successfully loaded {len(results_list)} / {len(dates)} date(s)")
    return results_list


def load_sectors_by_date(
    date: str | datetime,
    search_dir: Path,
    set_crs: str | None = None,
    to_crs: str | None = None,
    ...
    try:
        sectors_gdf, points_gdf = _load_geojson_pair(
            date=date,
        ...
        logger.error(f"Files not found for date {date} in {search_dir}.")
        return None, None
    except Exception as e:
        logger.error(f"Error loading GeoJSON for date {date} in {search_dir}: {e}")
        return None, None

    return sectors_gdf, points_gdf


def load_anomaly_by_date(
    date: str | datetime,
    search_dir: Path,
    set_crs: str | None = None,
    to_crs: str | None = None,
    ...
    try:
        anomaly_gdf, points_gdf = _load_geojson_pair(
            date=date,
            search_dir=search_dir,
        ...
        logger.error(
            f"Error loading anomaly GeoJSON for date {date} in {search_dir}: {e}"
        )
        return None, None

    return anomaly_gdf, points_gdf
...
# === Helper Functions ===#
def _parse_date(date: str | datetime, format="%Y-%m-%d") -> datetime:
...
def _load_geojson_pair(
    ...
    if set_crs is not None:
        polygon_gdf.set_crs(set_crs, allow_override=True, inplace=True)
        points_gdf.set_crs(set_crs, allow_override=True, inplace=True)

    if force_crs_to_none:
        polygon_gdf.set_crs(None, allow_override=True, inplace=True)
        points_gdf.set_crs(None, allow_override=True, inplace=True)

    if to_crs is not None:
        if polygon_gdf.crs is None or points_gdf.crs is None:
            raise ValueError(
                f"Cannot reproject to {to_crs} because input CRS is not set. "
                f"Set 'set_crs' to a valid CRS when loading the GeoDataFrames."
            )
        polygon_gdf = polygon_gdf.to_crs(to_crs)
        points_gdf = points_gdf.to_crs(to_crs)

    return polygon_gdf, points_gdf


def _has_complete_sector_pair(geojson_dir: Path, date_str: str) -> bool:
    """Return True if both polygon and points files exist for *date_str*."""
    polygon = geojson_dir / f"{date_str}_sectors_polygon.geojson"
    points = geojson_dir / f"{date_str}_sectors_points.geojson"
    return polygon.exists() and points.exists()
